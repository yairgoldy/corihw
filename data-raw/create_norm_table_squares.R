...
for(i in 1:nrow(pairs)){
  pairs$prob[i] <- s2(sqrt(pairs$rho[i]),pairs$gamma[i])
}
...
for(i in 1:nrow(triples)){
  rho12 <- sqrt(triples$rho1[i])
  rho13 <- sqrt(triples$rho2[i])
  rho23 <- sqrt(triples$rho3[i])
...
save(gamma_rho_pairs,gamma_rho_triples,file="norm_table_squares.Rdat")
